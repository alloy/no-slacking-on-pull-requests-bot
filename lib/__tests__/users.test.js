...
describe("Users", () => {
  ...
  it("updates the last known pull requests", () => {
    return Users.updateLastKnownPullRequests(alloy, ["eigen#42"]).then(() => {
      expect(alloy.lastKnownPullRequests).toEqual([])
      return Users.get(alloy.slackHandle).then(updatedUser => {
        expect(updatedUser.lastKnownPullRequests).toEqual(["eigen#42"])
      })
    })
  })
})
