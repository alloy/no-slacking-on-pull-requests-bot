...
describe("reportPullRequests", () => {
  ...
  it("reports that there are no pull requests, if requested", () => {
    reportPullRequests(bot, slackHandle, [], true)
    expect(convo.say).toBeCalledWith("You have no open PRs. Great job!", { action: "completed" })
  })
  ...
  it("reports formatted pull requests", () => {
    ...
    expect(convo.say).toBeCalledWith({ action: "completed", attachments: [formatPullRequest(pullRequest)] })
  })
})
