...
const GIT_VERSION = spawnSync("git", ["rev-parse", "HEAD"], { encoding: "utf-8" }).stdout.trim()
const GITHUB_REPO = spawnSync("git", ["remote", "get-url", "origin"], { encoding: "utf-8" }).stdout.trim()
  ...
  Users.get(message.user)
    ...
    .catch(error => {
      if (error.notRegistered) {
        bot.reply(message, "Love the enthusiasm, but please register first.")
      } else {
        console.error(error)
      }
    })
